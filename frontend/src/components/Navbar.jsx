...
const Navbar = ({ transparent = false }) => {
 ...
 const dispatch=useDispatch();
  const navItems = [
    ...
    { name: "Instructions to Author", path: "/instructions" },
  ...
  const handleSubmitClick = () => {
    if (isAuthenticated) {
      navigate('/IJPPI/author/new-submission');
    } else {
      navigate('/IJPPI/login');
    }
  };
  ...
  const handleDashboardClick = () => {
    if (user?.role) {
      const path = user.role === 'author' 
        ? `/IJPPI/${user.role}/dashboard`
        : `/IJPPI/${user.role}/journal/pharma`;
      navigate(path);
    }
  };
    ...
    <>
      <div>
              ...
              {isAuthenticated ? (
                <div className="relative group">
                  <div className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="font-[500] text-[14px] lg:text-[16px] xl:text-[18px] whitespace-nowrap">
                      {user?.firstName || 'User'}
                    </span>
                  </div>
        ...
        >
          ...
          >
            <img 
              src='https://res.cloudinary.com/duhadnqmh/image/upload/v1767786487/mslogo_gqwxzo.png' 
              ...
              <div key={item.path} className="relative">
                <p
                  ...
                  className={`font-[500] text-[14px] lg:text-[15px] xl:text-[16px] 2xl:text-[18px] hover:cursor-pointer hover:opacity-80 whitespace-nowrap transition-opacity pb-1
                    ${isCurrentPage(item.path) ? 'font-bold' : ''}`}
                >
                  {item.name}
                </p>
                  ...
                  <div key={item.path} className="relative">
                    <p
                      ...
                      className={`font-[500] text-[13px] sm:text-[14px] hover:cursor-pointer hover:opacity-80 py-2
                        ${isCurrentPage(item.path) ? `font-bold ${transparent ? 'text-blue-400' : 'text-blue-600'}` : ''}`}
                    >
                      {item.name}
                    </p>
                    {isCurrentPage(item.path) && (
